...
# Enable CORS for Flutter web and external apps
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
DATA_PATH = os.path.join(PROJECT_ROOT, "data", "processed", "master_df.csv")
...
def load_resources():
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(f"Forecasting model not found at {MODEL_PATH}")
...
def get_live_aqi():
    ...
    # Get the latest timestamp record for each unique location/ward
    latest_readings = []
...
def get_history_aqi(
    ...
    # Calculate cutoff time based on max timestamp in dataset
...
def get_forecast_aqi(
    ...
    for loc in wards_to_forecast:
        ...
        # Initialise recursive forecast variables
        ...
        for h in range(1, num_hours + 1):
            ...
            simulated_traffic = max(0, min(100, int(hourly_traffic)))
